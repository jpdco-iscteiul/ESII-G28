...
/**
 * Uses: HTMLTableBuilder htmlBuilder = new HTMLTableBuilder(null, true, 2, 3);
 * htmlBuilder.addTableHeader("1H", "2H", "3H"); htmlBuilder.addRowValues("1",
 * "2", "3"); htmlBuilder.addRowValues("4", "5", "6");
 * htmlBuilder.addRowValues("9", "8", "7"); String table = htmlBuilder.build();
...
public class HTMLTableBuilder {
	...
	public File create_file() {
		try {
			File myObj = new File("ESII.htm");
			PrintWriter myWriter = new PrintWriter(myObj);
			String tabela = build();
			myWriter.write(tabela);
			myWriter.close();
			return myObj;
		} catch (IOException e) {
